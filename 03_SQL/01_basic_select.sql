...
-- EMP 테이블에서 'Sales' 부서에 속하지 않은 직원들의 ID(emp_id), 이름(emp_name),  
-- 부서명(dept_name)을 조회.
select emp_id,
	   emp_name,
       dept_name
...
-- 급여(salary)가 $5,000을 넘는 직원의 ID(emp_id), 이름(emp_name), 
-- 급여(salary)를 급여가 높은 순서부터 조회
select  emp_id, emp_name, salary 
from    emp
where   salary > 5000
-- order by salary desc;
order by 3 desc; -- select 의 컬럼 순번으로 기준컬럼을 지정.
